...
serve(async (req) => {
  ...
  try {
    ...
    if (!email || !workspaceId) {
      return new Response(JSON.stringify({ error: 'Missing email or workspaceId payload' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (userError || !user) {
      return new Response(JSON.stringify({ error: 'Invalid auth session: ' + userError?.message }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!wsData || wsData.owner_id !== user.id) {
       return new Response(JSON.stringify({ error: 'You do not have permission to invite users to this workspace.' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    if (email === user.email) {
       return new Response(JSON.stringify({ error: 'You cannot invite yourself.' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!targetProfile) {
       return new Response(JSON.stringify({ error: `User ${email} is not registered yet. Ask them to sign up to the system first.` }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (insertError) {
        if (insertError.code === '23505') {
            return new Response(JSON.stringify({ error: 'User is already a member of this workspace.' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        return new Response(JSON.stringify({ error: 'Failed to add member: ' + insertError.message }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
